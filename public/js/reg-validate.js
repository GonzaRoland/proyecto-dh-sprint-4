window.onload = function(){
   ...
   select1.addEventListener("change", mostrarDatos)
    ...
    //console.log(firstNameErrorSpan)
    ...
    registerForm.onsubmit = function onFormSubmit(e) {
        ...
        if (validateRegisterForm()){
            ...
            swal('¡Gracias por registrarte!', 'Ya podés ingresar con tu email y contraseña', 'success')
            registerForm.submit()
    ...
    function validatePassword(data) {
        ...
        if (!passwordRegEx.test(data) || data === "" || data.length < 6 ) {
    ...
    function validatePasswordConfirm(data, repeat) {
        if (data != repeat) {
            passwordConfirmErrorSpan.innerHTML = passwordConfirmError
            passwordConfirm.setAttribute('class', 'form-control is-invalid')
            return false
        }
